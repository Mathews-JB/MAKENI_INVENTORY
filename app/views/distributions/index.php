...
<div class="mb-6 md:mb-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="text-xl md:text-2xl font-bold text-gray-900">Distribution Orders</h2>
            <p class="text-sm md:text-base text-gray-500 mt-1">Manage material distributions and orders</p>
        </div>
        <a href="<?php echo URL_ROOT; ?>/distributions/create" class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 md:px-6 py-2 md:py-3 rounded-lg hover:from-indigo-700 hover:to-purple-700 transition shadow-lg flex items-center justify-center transform hover:scale-105 text-sm md:text-base">
...
$totalRevenue = array_sum(array_map(function($o) { return $o->total_amount; }, $orders ?? []));
?>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
    ...
    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg md:rounded-xl p-4 md:p-6 text-white shadow-lg hover:shadow-xl transition transform hover:-translate-y-1">
        <div class="flex items-center justify-between mb-2 md:mb-4">
            <div class="bg-white bg-opacity-20 rounded-lg p-2 md:p-3">
                <i class="fas fa-truck-loading text-lg md:text-2xl"></i>
            </div>
        </div>
        <h3 class="text-2xl md:text-3xl font-bold mb-1"><?php echo $totalOrders; ?></h3>
        <p class="text-blue-100 text-xs md:text-sm">Total Orders</p>
    </div>
    ...
    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg md:rounded-xl p-4 md:p-6 text-white shadow-lg hover:shadow-xl transition transform hover:-translate-y-1">
        ...
        <h3 class="text-2xl md:text-3xl font-bold mb-1"><?php echo $completedOrders; ?></h3>
        <p class="text-green-100 text-xs md:text-sm">Completed</p>
    </div>
    ...
    <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg md:rounded-xl p-4 md:p-6 text-white shadow-lg hover:shadow-xl transition transform hover:-translate-y-1">
        ...
        <h3 class="text-xl md:text-3xl font-bold mb-1">K<?php echo number_format($totalRevenue, 0); ?></h3>
        <p class="text-purple-100 text-xs md:text-sm">Total Value</p>
    </div>
</div>
...
<div class="bg-white rounded-lg md:rounded-xl shadow-sm border border-gray-200 p-4 md:p-6 mb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
        <div class="flex-1">
            <div class="relative">
                <input type="text" id="searchOrders" placeholder="Search by order number or recipient..." 
                       class="w-full pl-10 pr-4 py-2 md:py-3 text-sm md:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                <i class="fas fa-search absolute left-3 top-3 md:top-4 text-gray-400 text-sm md:text-base"></i>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <select id="statusFilter" class="px-3 md:px-4 py-2 md:py-3 text-sm md:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
...
<div class="hidden md:block bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200" id="ordersTableBody">
                ...
                <tr class="hover:bg-gray-50 transition order-row" 
                    data-search="<?php echo strtolower($order->order_number . ' ' . $order->recipient_name); ?>"
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold shadow-md">
                                <?php echo strtoupper(substr($order->recipient_name, 0, 1)); ?>
                            </div>
                            <div class="ml-3">
                                <div class="text-sm font-semibold text-gray-900"><?php echo $order->recipient_name; ?></div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <?php
                        $statusConfig = [
                            'completed' => ['bg' => 'bg-green-100', 'text' => 'text-green-800', 'border' => 'border-green-200', 'icon' => 'fa-check-circle'],
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <a href="<?php echo URL_ROOT; ?>/distributions/viewOrder/<?php echo $order->id; ?>" 
                           class="text-indigo-600 hover:text-indigo-900 hover:bg-indigo-50 p-2 rounded-lg transition inline-block mr-2" title="View">
                            <i class="fas fa-eye"></i>
                        </a>
                        <a href="<?php echo URL_ROOT; ?>/distributions/receipt/<?php echo $order->id; ?>" 
                           class="text-green-600 hover:text-green-900 hover:bg-green-50 p-2 rounded-lg transition inline-block" title="Receipt">
    ...
    <div class="text-center py-16">
        ...
        <h3 class="text-lg font-semibold text-gray-900 mb-2">No Distribution Orders Found</h3>
        <p class="text-gray-500 mb-6">Create your first distribution order to get started</p>
        <a href="<?php echo URL_ROOT; ?>/distributions/create" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition shadow-lg">
            <i class="fas fa-plus mr-2"></i>
            Create First Order
        </a>
    </div>
...
<div class="md:hidden space-y-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 order-card" 
            ...
            <div class="flex items-start justify-between mb-3">
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        <div class="text-sm font-semibold text-gray-900"><?php echo $order->recipient_name; ?></div>
                        <div class="text-xs font-mono text-indigo-600"><?php echo $order->order_number; ?></div>
                    </div>
                </div>
                <?php
                $statusConfig = [
    ...
    <div class="text-center py-12 bg-white rounded-lg border border-gray-200">
        ...
        <h3 class="text-base font-semibold text-gray-900 mb-2">No Distribution Orders</h3>
        <p class="text-sm text-gray-500 mb-4">Create your first distribution</p>
        <a href="<?php echo URL_ROOT; ?>/distributions/create" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg text-sm">
            <i class="fas fa-plus mr-2"></i>
            Create Distribution
        </a>
    </div>
